...
ggplot has endless options to specify the colors labels, scales, axes, themes, legends,and more. Once you know how to use the functions, you can apply them to style your work according to data viz best practices. Building a ggplot best shown through examples
...
langs_by_boro <- read_csv("langs_by_boro.csv")
...
```{r, message = F}
ggsave("output/language_by_boro_faceted.png") #ggsave defaults to the last plot you ran

ggsave(filename = "output/language_by_boro.png", plot = barchart) #you can also assign the plot and specify what to save
```
